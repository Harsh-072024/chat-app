...
export const deny = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!request || request.receiver !== currentUser._id) {
      throw new ConvexError("There was an error during this request");
    }
...
export const accept = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("conversationMembers", {memberId: currentUser._id,
        conversationId
    })
    await ctx.db.insert("conversationMembers", {memberId: request.sender,
        conversationId
    })
    await ctx.db.delete(request._id);
  },
